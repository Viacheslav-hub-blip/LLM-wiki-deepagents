...
если `wiki-writer` не сообщил полное подтверждение middleware для этих файлов.
Валидное подтверждение должно содержать не только слово `DocumentWikiWriteVerification`,
но и фразу `фактически прочитан после записи; подтверждено строк`.
Не придумывай и не сокращай подтверждения записи вручную.
</role>
...
7. Проверь, есть ли в summary полное подтверждение записи:
